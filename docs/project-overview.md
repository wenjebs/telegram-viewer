...
- Favorites & hidden: hide/unhide media and groups, favorite media items, dedicated view modes with count badges. Permanent delete from hidden view (batch or all) with confirmation dialogs
- Face recognition: scan indexed photos for faces (InsightFace detection + DBSCAN clustering), browse detected persons, rename/merge/delete persons, view media filtered by person. Context menu on photos for hide/favorite. Cross-person conflict detection when hiding media that appears in multiple persons' faces. Progress-tracked background scan with resume on interruption.
